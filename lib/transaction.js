...
exports.deleteTrans = function(req, res){
	...
	Transaction.findOne({'_id' : req.body.tID}).populate('listing').exec(function(err, t){
		if(err) return console.log(err);
		if(!t) res.send("BAD");
		t.status = "failed";
		t.save(function(err, t){
			if(err){
				...
				User.findOne({userName : receiver}, function(err, u){
					...
					var alert = new Object();
					alert.type = "canAlert";
					alert.message = "" + req.user.userName + " deleted your interaction! The listing was reset as available";
					alert.active = true;
					u.alerts.push(alert);
					u.save(function(err, u){
						if(err) return err;
						Listing.findOne({"_id": t.listing._id}, function(err, l){
							l.status = "available";
							l.save(function(err, t){
								if(err) return err;
								console.log(l);
								res.send("OK");
							});
...
exports.completeTrans = function(req, res){
	res.setHeader("Content-Type", "text/html");
	body = JSON.stringify(req.body);
	console.log("ID:  " + req.body.tID);
		...
		t.save(function(err, t){
			if(err){
				...
				User.findOne({userName : receiver}, function(err, u){
					...
					var alert = new Object();
					alert.type = "canAlert";
					alert.message = "" + req.user.userName + " deleted your interaction!";
					alert.active = true;
					u.alerts.push(alert);
					u.save(function(err, u){
						if(err) return err;
